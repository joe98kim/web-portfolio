...
(() => {
  ...
  const stylesheet =
    ...
    // touch-action:none only while the reframe/pan UI is actually active —
    // scoped to :host([data-reframe]), not the base .frame img rule, so a
    // normal page visitor can still scroll past an image with a touch drag
    // (a bare `touch-action:none` on every image blocks that everywhere,
    // all the time, for a pan gesture that's only ever reachable from the
    // editor's double-click reframe flow).
    ...
    '.ring{position:absolute;inset:0;pointer-events:none;border:none;' +
